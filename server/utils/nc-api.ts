...
export function normalizeNcUrl(rawUrl: string): string {
  ...
  if (url.username || url.password) {
    throw createError({
      statusCode: 400,
      statusMessage: "Nextcloud URL must not include credentials",
    });
  }
...
function getBasicAuthHeader(credentials: NcCredentials) {
  const token = Buffer.from(`${credentials.loginName}:${credentials.appPassword}`).toString(
    "base64",
  );
...
function mergeRequestHeaders(credentials: NcCredentials, initHeaders?: HeadersInit): Headers {
