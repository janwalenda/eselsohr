...
async function getDavResponseForUrl(
  event: H3Event,
  davUrl: string,
  init: RequestInit = {},
) {
...
export async function writePageContent(
  ...
  if (!response.ok) {
    throw createError({
      statusCode: response.status,
      statusMessage: response.status === 412
...
<d:propfind xmlns:d="DAV:" xmlns:oc="http://owncloud.org/ns" xmlns:nc="http://nextcloud.org/ns">
  ...
  if (!fileId) {
    throw createError({ statusCode: 502, statusMessage: 'Nextcloud did not return a file id for this page' })
  }
