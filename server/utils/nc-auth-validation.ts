...
export type CollectivesWriteValidationFailureReason =
  "no_collectives_app" | "no_editable_collective" | "write_probe_failed";
...
export async function validateCollectivesWriteAccess(
  ...
  const editableCollectives = (collectivesData.collectives ?? []).filter(
    (collective) => collective.canEdit !== false,
  );
  ...
  return {
    ...
    reason: "write_probe_failed",
    message:
      "Diese Zugangsdaten können in Collectives keine Seiten erstellen. Bitte nutze ein manuell erstelltes App-Passwort.",
  };
}
