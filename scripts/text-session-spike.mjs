...
async function main() {
  ...
  console.log({ status: sync.status, steps: sync.json?.steps?.length, sessions: sync.json?.sessions?.length })
  ...
  setInterval(async () => {
    ...
    if (steps.length > 0) {
      ...
      console.log(`[${new Date().toISOString()}] received ${steps.length} step(s), version -> ${version}`)
    }
    const sessions = poll.json?.sessions ?? []
    if (sessions.length > 1) {
      console.log(`  collaborators: ${sessions.map(s => s.displayName || s.guestName || s.userId).join(', ')}`)
    }
  }, 1000)
}
