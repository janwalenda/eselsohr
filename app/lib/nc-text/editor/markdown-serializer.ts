...
function serializeTable(state: MarkdownSerializerState, node: ProseMirrorNode) {
  ...
  state.ensureNewLine()
  state.write(`| ${pad(headerRow).map(() => '---').join(' | ')} |`)
  state.ensureNewLine()
...
const nodes: Record<string, (state: MarkdownSerializerState, node: ProseMirrorNode, parent: ProseMirrorNode, index: number) => void> = {
